...
SimSummary <- function(StoreSims, RootName, ODType = NA, 
	failCutEst = NA, failCutSE = NA, failCutOD = NA)
{
  ...
  CIcover95 <- sum(StoreSims[,"TrueAbun"] > exp(log(StoreSims[,EstCol]) -
      qnorm(.975)*StdError/StoreSims[,EstCol]) &
    StoreSims[,"TrueAbun"] < exp(log(StoreSims[,EstCol]) +
      qnorm(.975)*StdError/StoreSims[,EstCol]))/length(StoreSims[,1])
  CIcover90 <- sum(StoreSims[,"TrueAbun"] > exp(log(StoreSims[,EstCol]) -
      qnorm(.95)*StdError/StoreSims[,EstCol]) &
    StoreSims[,"TrueAbun"] < exp(log(StoreSims[,EstCol]) +
      qnorm(.95)*StdError/StoreSims[,EstCol]))/length(StoreSims[,1])
  CIcover80 <- sum(StoreSims[,"TrueAbun"] > exp(log(StoreSims[,EstCol]) -
      qnorm(.9)*StdError/StoreSims[,EstCol]) &
    StoreSims[,"TrueAbun"] < exp(log(StoreSims[,EstCol]) +
      qnorm(.9)*StdError/StoreSims[,EstCol]))/length(StoreSims[,1])
  }
