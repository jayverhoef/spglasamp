library(maptools)
library(aecisp)
setwd("/media/Hitachi2GB/00NMML/GlacierPhotoSampling/aecisp/aecisp/inst/data/Sim3")
...
# ------------------------------------------------------------------------------
  ...
  fCnt5 <- floor(.05*length(SpatialCountData[,"Count"]))
  fCnt5 <- max(fCnt5,1)
  fCnt10 <- floor(.10*length(SpatialCountData[,"Count"]))
  fCnt10 <- max(fCnt10,1)
  fCnt20 <- floor(.15*length(SpatialCountData[,"Count"]))
  fCnt20 <- max(fCnt20,1)
  fCnt30 <- floor(.20*length(SpatialCountData[,"Count"]))
  fCnt30 <- max(fCnt30,1)
...
#  text(1,0.5, paste("Elapsed Time", iter.time - Start.Time), cex = 2)
  ...
  EstOut1 <- isppifac1(SurvAreaBoundaryClassOwin = SurvArea,
    ...
    CountVarName = "Count", AreaVarName = "Area",
    xColName = "centroidx", yColName = "centroidy",
    nNodesRequestC = max(ceiling(fCnt10/5),2), nNodesRequestF = floor(4*fCnt10/4),
		ScaleFactorDens = MaxDens.fix,
    PredictionGrid = predgrd, ProportionUnsurveyed = ProportionUnsurveyed,
    PercentZero = 75)
